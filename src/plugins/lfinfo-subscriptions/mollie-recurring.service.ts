...
export class MollieRecurringService {
  ...
  async start(ctx: RequestContext, consent: boolean) {
    ...
    const activeOrder = await this.activeOrders.getActiveOrder(ctx, undefined); const order = activeOrder && await this.orders.findOne(ctx, activeOrder.id, ['customer', 'lines', 'lines.productVariant']); if (!order?.customer || order.lines.length === 0) throw new Error('Panier introuvable');
    const plans = await this.connection.getRepository(ctx, SubscriptionPlan).find({ where: { active: true } });
